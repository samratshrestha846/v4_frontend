...
import { BOOLEAN_STATUS_ACTIVE } from '../../../constants/constants';
...
export default function useCreateSupplement() {
  ...
  const createSupplement = (fromData: FormData) => {
    return supplements.createSupplement(fromData);
  };
  ...
  const {
    ...
    defaultValues: {
      is_active: BOOLEAN_STATUS_ACTIVE,
      methane_reducing_factor: 0,
    },
  });
  ...
  const onSubmit = async (formData: any) => {
    ...
    await createSupplementMutation.mutate(formData);
  };
