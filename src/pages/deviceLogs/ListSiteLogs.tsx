...
import { DEVICE_LIST, DEVICE_SITE_LOGS_LIST } from '../../constants/path';
...
const ListSiteLogs: React.FC = () => {
  const { id } = useParams();
  const [searchParams] = useSearchParams();

  const deviceSerialNo = searchParams.get('dsn');
  ...
  const {
    ...
    action,
    setAction,
  } = useListDeviceLogs({ deviceId: Number(id) });
  ...
  return (
    <>
      <PageTitle
        breadCrumbItems={[
          { label: 'Devices', path: DEVICE_LIST },
          { label: 'Site Logs', path: DEVICE_SITE_LOGS_LIST, active: true },
        ]}
        title={deviceSerialNo || 'Site Logs'}
      />
