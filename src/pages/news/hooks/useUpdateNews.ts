...
export default function useUpdateNews() {
  ...
  const createNews = (formData: NewsFormFields) => {
    return news.updateNews(formData, Number(id));
  };
  ...
  useEffect(() => {
    if (newsDetail) {
      reset({
        ...
        is_published: newsDetail.is_published,
        published_on: new Date(newsDetail.published_on),
        url_link: newsDetail.url_link,
      });
    }
  }, [newsDetail]);
