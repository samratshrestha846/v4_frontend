...
export default function useUpdateCropAssignedToBlock({
  toggleModal,
  refetchBlocks,
  cropableDetail,
}: Props) {
  ...
  const updateCropAssignedToBlock = (formData: CropableFormValues) => {
  ...
  const updateCropAssignedToBlockMutation = useMutation({
    mutationKey: ['update-crop-assigned-to-block'],
    mutationFn: updateCropAssignedToBlock,
    onSuccess,
    onError,
  });
  ...
  const onSubmit = async (formData: CropableFormValues) => {
    ...
    updateCropAssignedToBlockMutation.mutate(formData);
  };
  ...
  return {
    ...
    updateCropAssignedToBlockMutation,
    submitted,
  };
}
