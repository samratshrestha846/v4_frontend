...
export default function useAssignCropToSubBlock({
  toggleModal,
  refetchSubBlocks,
  subBlockDetail,
}: Props) {
  ...
  const assignCropToSubBlock = (formData: CropableFormValues) => {
    return cropable.assignCropToSubBlock(formData, Number(subBlockDetail?.id));
  };
  ...
  const assignCropToSubBlockMutation = useMutation({
    mutationKey: ['assign-crop-to-sub-block'],
    mutationFn: assignCropToSubBlock,
    onSuccess,
    onError,
  });
  ...
  const onSubmit = async (formData: CropableFormValues) => {
    ...
    assignCropToSubBlockMutation.mutate(formData);
  };
